...
class Graph{
    ...
    public:
        ...
        void topologicalSortBFS(){ // outputs linear ordering of vertices of the graph such that for every edge (u->v) vertex u comes before v in the ordering. // works  on DAG Directed Acyclic Graph 
            map<T,int>indegree; 
            map<T,bool>visited; 
            queue<T>q; 
            for(auto it:adjList){
